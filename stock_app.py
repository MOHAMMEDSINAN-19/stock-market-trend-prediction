...
with col_title:
    st.markdown(
        "<h2 style='color:#0F172A;font-weight:800;margin-bottom:0;'>StockVision AI</h2>",
        unsafe_allow_html=True
    )
    st.markdown(
        "<p style='color:#475569;font-size:15px;margin-top:4px;'>Multi-signal ML price prediction · S&P 500 · Confidence intervals · Smart recommendations</p>",
        unsafe_allow_html=True
    )
with col_time:
    st.markdown(
        f"<p style='color:#334155;font-weight:800;margin-bottom:4px;'>{datetime.now().strftime('%b %d, %Y')}</p>",
        unsafe_allow_html=True
    )
    st.markdown(
        f"<p style='color:#334155;font-style:italic;margin-top:0;'>{datetime.now().strftime('%H:%M')} local</p>",
        unsafe_allow_html=True
    )
...
with st.sidebar:
    ...
    company_mapping = {
    "ADOBE": {"name": 1, "company": 0},
    "AMZN": {"name": 2, "company": 1},
    "APPLE": {"name": 0, "company": 2},
    "CSCO": {"name": 3, "company": 3},
    "GOOGL": {"name": 5, "company": 4},
    "IBM": {"name": 6, "company": 5},
    "INTC": {"name": 7, "company": 6},
    "FB": {"name": 4, "company": 7},
    "MSFT": {"name": 8, "company": 8},
    "NFLX": {"name": 9, "company": 9},
    "NVDA": {"name": 10, "company": 10},
    "ORCL": {"name": 11, "company": 11},
    "PYPL": {"name": 12, "company": 12}
    }
    ...
    name_value = company_mapping[selected_company]["name"]
    company_value = company_mapping[selected_company]["company"]
...
k1.metric("Model",      "Linear Reg.")
k2.metric("Dataset",    "Stock ML")
k3.metric("R² Score",   "0.9996")
...
with left:
    st.markdown("<h4 style='color:#0F172A;font-weight:800;'>🚀 Prediction Engine</h4>", unsafe_allow_html=True)
    st.markdown("<p style='color:#475569;'>Set market inputs in the sidebar, then run the model.</p>", unsafe_allow_html=True)
    ...
    if predict_btn:
        ...
        <div class="price-card" style="border-color:{color_hex}40;">
            <p style="margin:0;font-size:10px;letter-spacing:.1em;
                      text-transform:uppercase;color:#334155;font-weight:800;">
                Predicted Next Close
            </p>
            <p style="margin:6px 0 2px;font-size:40px;font-weight:700;
                      color:#0F172A;letter-spacing:-1px;font-family:'JetBrains Mono',monospace;">
                ${predicted_price:.2f}
            </p>
            ...
            <p style="margin:8px 0 0;font-size:12px;color:#475569;">
                95% CI: <span style="font-family:'JetBrains Mono',monospace;color:#334155;">
...
with right:
    ...
    with tab1:
        ...
        fig.update_layout(
            title=dict(text="Session Overview + Prediction", font=dict(color="#0F172A", size=15), x=0.0),
            plot_bgcolor="#F8FAFC",
            paper_bgcolor="#F8FAFC",
            font=dict(color="#334155"),
            xaxis=dict(showgrid=False, tickfont=dict(color="#64748B"),
                       linecolor="#CBD5E1", rangeslider_visible=False),
            yaxis=dict(showgrid=True, gridcolor="#E2E8F0",
                       tickfont=dict(color="#64748B"), tickprefix="$"),
            height=390,
        ...
        fig2.update_layout(
            title=dict(text="Price Comparison", font=dict(color="#0F172A", size=14), x=0.0),
            plot_bgcolor="#F8FAFC", paper_bgcolor="#F8FAFC",
            font=dict(color="#334155"),
            xaxis=dict(showgrid=False, tickfont=dict(color="#64748B"), linecolor="#CBD5E1"),
            yaxis=dict(showgrid=True, gridcolor="#E2E8F0",
                       tickfont=dict(color="#64748B"), tickprefix="$"),
    ...
    with tab2:
        if "signals" not in st.session_state:
            ...
            st.markdown("<h5 style='color:#0F172A;font-weight:800;'>Signal Breakdown</h5>", unsafe_allow_html=True)
            ...
            # Signal gauge chart
            st.markdown("<h5 style='color:#0F172A;font-weight:800;'>Overall Signal Gauge</h5>", unsafe_allow_html=True)
            fig_gauge = go.Figure(go.Indicator(
                ...
                gauge=dict(
                    ...
                    bordercolor="#131F3A",
                    steps=[
                        dict(range=[0, 35],  color="#FEE2E2"),
                        dict(range=[35, 45], color="#FFEDD5"),
                        dict(range=[45, 55], color="#E0F2FE"),
                        dict(range=[55, 65], color="#DCFCE7"),
                        dict(range=[65, 100], color="#BBF7D0"),
                    ],
                    threshold=dict(
                ...
                number=dict(font=dict(color="#0F172A", size=36,
...
# ── TECHNICAL SNAPSHOT ───────────────────────────────────────────────────────
st.divider()
st.markdown("<h4 style='color:#0F172A;font-weight:800;'>Technical Snapshot</h4>", unsafe_allow_html=True)
